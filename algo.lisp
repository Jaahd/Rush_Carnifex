  ...
  (loop for i from 0 to (- width 1) do
        (loop for j from 0 to (- height 1) do
              ...
              (if (= (cell-gen (aref grid i j)) 1)
                ...
                (sdl:draw-box (sdl:rectangle :x (round (* 1200 (/ i width))) :y (round (* 1200 (/ j height))) :w (round (/ 1200 width)) :h (round (/ 1200 height))) :color *random-color*)
                (format t "ALIVE~d ~d~&" i j)
                )
              )
        )
  (sleep sl_time)
  ...
  (let ((sl_time 0.1) (p_time nil))
    (sdl:with-init ()
                   ...
                   (sdl:with-events ()
                                    ...
                                    (:mouse-button-up-event (:state button :x mouse-x :y mouse-y)
                                                            (if (= button 1)
                                                              (sdl:draw-box (sdl:rectangle :x mouse-x :y mouse-y :w
                                                                                           (round (/ 1200 width)) :h (round (/ 1200 height)))
                                                                            :color *random-color*)
                                                              (format t "~&-- MOUSE-X -- ~d~&-- MOUSE-Y -- ~d~&" mouse-x mouse-y)
                                                              ))
